...
import discord from discord import app_commands from discord.ext import commands import random from datetime import datetime import json import os
...
def load_command_permissions(): """Load command permissions from file""" global COMMAND_PERMISSIONS try: if os.path.exists('command_permissions.json'): with open('command_permissions.json', 'r') as f: COMMAND_PERMISSIONS = json.load(f) except: COMMAND_PERMISSIONS = {}

Load permissions on module import
load_command_permissions()

def find_optimal_teams(player_ids: list, player_mmrs: dict) -> tuple: """Find the optimal team split using exhaustive search. Returns (red_team, blue_team, mmr_diff)""" best_diff = float('inf') best_team1 = None best_team2 = None
...
def log_action(message: str): """Log actions""" from searchmatchmaking import log_action as queue_log queue_log(message)

async def get_player_mmr(user_id: int) -> int: """Get player MMR""" import STATSRANKS stats = STATSRANKS.get_player_stats(user_id) if stats and 'mmr' in stats: return stats['mmr'] return 1500

def setup_commands(bot: commands.Bot, PREGAME_LOBBY_ID: int, POSTGAME_LOBBY_ID: int, QUEUE_CHANNEL_ID: int): """Setup all bot commands"""
